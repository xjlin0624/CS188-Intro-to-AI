...
class PerceptronModel(object):
    ...
    def get_prediction(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        scaler = nn.as_scalar(self.run(x))
        return 1 if scaler >= 0 else -1
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        output = nn.AddBias(nn.Linear(layer, self.w2), self.b2)
        return output
        ...
        while loss >= .015:
            for x, y in dataset.iterate_once(batch_size):
                ...
                for i in range(len(self.params)):
                    self.params[i].update(gradient[i], -0.05)
    ...
    def train(self, dataset):
        ...
        while accuracy < 0.975:
            for x, y in dataset.iterate_once(batch_size):
                ...
                loss = nn.as_scalar(loss)
                for i in range(len(self.params)):
                    self.params[i].update(gradient[i], -0.5)
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.params = [self.w0, self.b0, self.w, self.w_hidden,
                       self.b, self.w_output, self.b_output]
    ...
    def run(self, xs):
        ...
        for char in xs[1:]:
            h_i = nn.ReLU(nn.AddBias(nn.Add(nn.Linear(char, self.w), nn.Linear(h_i, self.w_hidden)), self.b))
        output = nn.AddBias(nn.Linear(h_i, self.w_output), self.b_output)
        return output
