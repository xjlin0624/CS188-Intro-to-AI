...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[OBS0] = range(abs(X_RANGE+ Y_RANGE + MAX_NOISE - 1))
    variableDomainsDict[OBS1]= range(abs(X_RANGE+ Y_RANGE + MAX_NOISE - 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        #iterate hidden vasr & peform join + eliminate on it
        for eliminationVariable in eliminationOrder:
            currentFactorsList, newFactor = joinFactorsByVariable(currentFactorsList, eliminationVariable)
            #new factor only has 1 unconditioned var, discard
            if len(newFactor.unconditionedVariables()) > 1:
                elim = eliminate(newFactor, eliminationVariable)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #iterate updates over 
        # self.allPositions
        #update self.beliefs -- probabiity ghost is at location
        jail = self.getJailPosition()
        pacman = gameState.getPacmanPosition()
        allGhostPos = self.allPositions
        beliefs = self.beliefs

        for ghost in allGhostPos:
            prior = self.getObservationProb(noisyDistance=observation, pacmanPosition=pacman, ghostPosition=ghost, jailPosition=jail)
            beliefs[ghost] *= prior
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in allGhostPos:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for newPos, prob in newPosDist.items():
                newBeliefs[newPos] += beliefs[oldPos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        particlesPerPos = int(self.numParticles / len(self.legalPositions))
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        jail = self.getJailPosition()
        for particle in self.particles:
            part_dist = self.getObservationProb(observation, pos, particle, jail)
            dist[particle] += part_dist
        if dist.total() == 0:
            self.initializeUniformly(gameState)
        else:
            dist.normalize()
