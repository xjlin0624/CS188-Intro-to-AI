...
def sentence3() -> Expr:
    ...
    statement_1 = alive_1 % disjoin([(alive_0 & ~killed_0), (~alive_0 & born_0)])
    statement_2 = ~(alive_0 & born_0)
    return conjoin([statement_1, statement_2, born_0])
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # kb entails d if kb entails not d is unsatisfiable
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for i in range(len(literals)):
        for j in range(i+1, len(literals)):
            combo.append(disjoin(~literals[i], ~literals[j]))
    return conjoin(combo)
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    return conjoin(PropSymbolExpr(pacman_str, x, y, time=now) % atLeastOne(possible_causes))
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # For all x,y in all_coords, if a wall is at x,y, then Pacman is not at x,y at t
    # not (a wall at x, y) OR pacman not at x,y at t
    for coord in all_coords: 
        x, y = coord
        wall = PropSymbolExpr(wall_str, x, y)
        pacman = PropSymbolExpr(pacman_str, x, y, time=t)
        pacphysics_sentences.append(wall >> ~pacman)
    # pacman at exactly one of the non_outer_wall_coords at timestep t
    locations = []
    for (x,y) in non_outer_wall_coords: 
        locations.append(PropSymbolExpr(pacman_str, x, y, time=t))
    pacphysics_sentences.append(exactlyOne(locations))
    # pacman takes exactly one of the four actions in DIRECTIONS in timestep t
    moves = [
        PropSymbolExpr('North', time=t), PropSymbolExpr('South', time=t), 
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    # allLegalSuccessorAcioms for transitions
    KB.append(allLegalSuccessorAxioms(1, walls_grid, non_outer_wall_coords))
    # Pacman's current location (x0, y0)
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    # Pacman takes action0 and action1
    KB.append(PropSymbolExpr(action0, time=0))
    KB.append(PropSymbolExpr(action1, time=1))

    model1 = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, x1, y1, time=1)) # at x1,y1
    model2 = findModel(conjoin(KB) & ~PropSymbolExpr(pacman_str, x1, y1, time=1)) # NOT at x1,y1
...
def positionLogicPlan(problem) -> List:
    ...
    # initial knowledge: pacman's initial location at timestamp 0
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    ...
    for t in range(50):
        ...
        # find model that would satisfy assignment for KB
        model = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, xg, yg, time=t))
        # if yes return sequence of actions from start to goal
        if model:
            return extractActionSequence(model, actions)
        # pacman takes exactly one action per timestamp
        moves = [
            PropSymbolExpr('North', time=t), PropSymbolExpr('South', time=t), 
            PropSymbolExpr('East', time=t), PropSymbolExpr('West', time=t)]
        KB.append(exactlyOne(moves))
        # transition model sentences for all possible pacman positions in non_wall_coords
        for (x,y) in non_wall_coords: 
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        # find model that would satisfy assignment for KB
        model = findModel(conjoin(KB) & ~atLeastOne(food_list))
        # if yes return sequence of actions from start to goal
        if model:
            return extractActionSequence(model, actions)
        # pacman takes exactly one action per timestamp
        ...
        for (x,y) in non_wall_coords: 
            ...
            KB.append(
                conjoin(~PropSymbolExpr(pacman_str, x, y, time=t), PropSymbolExpr(food_str, x, y, time=t)) 
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for (x,y) in non_outer_wall_coords:
            ...
            model = findModel(conjoin(conjoin(KB), pac))
            if model: 
                possible_locations.append((x,y))
            if entails(conjoin(KB), pac):
                KB.append(pac)
            elif entails(conjoin(KB), ~pac): 
                KB.append(~pac)
